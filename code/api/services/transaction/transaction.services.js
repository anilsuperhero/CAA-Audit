...
module.exports = {
  ...
  create: async (req, res) => {
    try {
      const { id, token, type, amount, stage } = req.body;
        ...
        stripe.paymentIntents.create(charge, async (error, data) => {
          if (error) {
            ...
            await transactions.save(async function (err, result) {
              ...
              if (auditData.type === "Verification") {
                ...
                await Audits.updateOne(
                  { _id: auditData._id },
                  {
                    ...
                    is_advance: 1,
                    reminderInfo: reminderInfo,
                    status: stage === 1 ? "INVOICE_PAID" : "INVOICE_PAID_STAGE",
                    status_view:
                      stage === 1 ? "INVOICE_PAID" : "INVOICE_PAID_STAGE",
                  }
                );
              }
              ...
              var users = await Users.findOne({ _id: auditData.company_id });
              var message =
                "You have paid successfully payment for. audit reference number is #" +
                auditData.title;
              ...
              var message =
                users.company_name +
                " has paid audit charges for quote reference number #" +
                auditData.title;
              if (stage === 2) {
                message =
                  users.company_name +
                  " has paid final payment for quote reference number #" +
                  auditData.title;
              }
              var notification = {};
              ...
              notification.message = message;
              notification.action =
                "audit-request?type=pop&audit_id=" + auditData._id;
              notification.title = "Audit Request";
              ...
              return res.json(responseData("PAYMENT_CREATE", { slug: id }));
            });
          }
        });
      });
  ...
  transfer: async (req, res) => {
    try {
      const { id, type, amount, remarks, stage } = req.body;
      const reciptNumber = "" + generateOTP() + generateOTP();
      await Audits.findOne({ _id: id }, async (err, auditData) => {
        if (err) {
          return res.status(422).json(responseData("DATA_NOT_FOUND", {}, 422));
