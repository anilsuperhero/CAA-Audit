...
module.exports = {
  download: async (req, res) => {
    try {
      ...
      var httpURL =
        ...
        "v1/keyPersonal/personnels?CompanyId=" +
        companyId;
      var wkhtmltopdf = require("wkhtmltopdf");
      var fs = require("fs");
      await wkhtmltopdf(httpURL, { pageSize: "letter" }).pipe(
        await fs.createWriteStream("public/personnels/" + companyId + ".pdf")
      );
      let pdfURL = process.env.API_PATH + "personnels/" + companyId + ".pdf";
      return res.json(
  ...
  personnels: async (req, res) => {
    try {
      let { page, sort, direction, keyword, CompanyId } = req.query;
      keyword = _.trim(keyword);
      var regexString = "";
      const sortOptions = {
        [sort || "created_at"]: direction === "desc" ? 1 : -1,
