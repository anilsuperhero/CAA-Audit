...
export function loadAuditorListingDataSuccess(auditorListing) {
  return { type: types.AUDITOR_LISTING_DATA, auditorListing };
}
...
export const auditorListingData = () => async (dispatch) => {
  try {
    const response = await agent.get(API.LOAD_AUDITOR + "/listing");
    dispatch(loadAuditorListingDataSuccess(response.data.data));
  } catch (error) {
    dispatch(
      tosterStatusAction({
        open: true,
