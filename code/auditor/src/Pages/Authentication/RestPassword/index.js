...
const Index = (props) => {
  ...
  const { userAuditorParams } = useSelector((state) => ({
    userAuditorParams: state.userAuditorParams,
  }));
  const { register, errors, handleSubmit } = useForm({
    defaultValues: userAuditorParams,
  });

  const onSubmit = (data) => {
    let params = data;
    params.email = userAuditorParams.email;
  ...
  const resendOtpClick = (e) => {
    e.preventDefault();
    const data = {};
    data.email = userAuditorParams.email;
